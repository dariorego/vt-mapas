...
$db = new Database();
$currentPage = 'forma_pagamento.php';
...
if (isset($_GET['ajax']) && $_GET['ajax'] === 'list') {
    ...
    try {
        ...
        if (!in_array($sortCol, ['id','descricao','situacao'])) $sortCol = 'id';
        $limit  = max(1, min(200, (int)($_GET['limit']  ?? 25)));
        $offset = max(0, (int)($_GET['offset'] ?? 0));
        $params = [];
        $where  = "FROM forma_pagamento WHERE 1=1";
        if (!empty($search)) {
...
if (isset($_GET['ajax']) && $_GET['ajax'] === 'get') {
    ...
    try {
        ...
        $row = $db->queryOne("SELECT id, descricao, situacao FROM forma_pagamento WHERE id = ?", [$id]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax']) && $_POST['ajax'] === 'save') {
    ...
    try {
        ...
        if ($id) {
            $db->execute("UPDATE forma_pagamento SET descricao=:d, situacao=:s WHERE id=:id", [':d'=>$descricao,':s'=>$situacao,':id'=>$id]);
            echo json_encode(['success'=>true,'message'=>'Forma de pagamento atualizada']);
        } else {
            $db->execute("INSERT INTO forma_pagamento (descricao, situacao) VALUES (:d, :s)", [':d'=>$descricao,':s'=>$situacao]);
            echo json_encode(['success'=>true,'message'=>'Forma de pagamento criada']);
        }
    } catch (Exception $e) { echo json_encode(['success'=>false,'error'=>$e->getMessage()]); }
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax']) && $_POST['ajax'] === 'delete') {
    ...
    try {
        $id = intval($_POST['id'] ?? 0);
        if (!$id) throw new Exception('ID inválido');
        $uso = $db->queryOne("SELECT COUNT(*) as n FROM remessa WHERE forma_pagamento_id = ?", [$id]);
        if ($uso && $uso['n'] > 0) throw new Exception("Não é possível excluir: {$uso['n']} remessa(s) vinculada(s)");
        $db->execute("DELETE FROM forma_pagamento WHERE id = ?", [$id]);
...
<head>
    ...
    <title>Formas de Pagamento - <?php echo EMPRESA_NOME; ?></title>
    <style>
        :root {
        ...
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .action-btn.delete { background:#FEF2F2; color:#DC2626; } .action-btn.delete:hover { background:#FEE2E2; }
...
<main class="main-content page-with-sidebar">
    <div class="page-header">
        ...
        <button class="btn btn-primary" onclick="openModal()">➕ Nova Forma</button>
    </div>
    ...
    <div class="modal">
        ...
        <div class="modal-body">
            ...
            <div class="form-group">
                ...
                <input type="text" id="recDescricao" placeholder="Ex: Pix, Dinheiro, Cartão...">
            </div>
            <div class="form-group">
...
<div class="delete-overlay" id="deleteModal">
    <div class="delete-modal">
        <h3>🗑️ Excluir Forma de Pagamento</h3>
...
<script>
    ...
    function sortBy(col) { currentDir = currentSort===col ? (currentDir==='ASC'?'DESC':'ASC') : 'ASC'; currentSort=col; currentPage=1; loadData(); }
    function clearFilters() { document.getElementById('searchInput').value=''; document.getElementById('perPageSelect').value='25'; currentPage=1; loadData(); }
    ...
    async function loadData() {
        ...
        try {
            const res  = await fetch(`forma_pagamento.php?ajax=list&search=${encodeURIComponent(search)}&sort=${currentSort}&dir=${currentDir}&limit=${limit}&offset=${offset}`);
            const data = await res.json();
            if (!data.success) throw new Error(data.error);
            if (!data.data.length) {
                tbody.innerHTML = '<tr><td colspan="4"><div class="empty-state"><div class="icon">💳</div><p>Nenhuma forma de pagamento encontrada</p></div></td></tr>';
            } else {
            ...
            updateSortHeaders();
        } catch(e) { tbody.innerHTML='<tr><td colspan="4" class="loading">❌ Erro ao carregar</td></tr>'; showToast(e.message,'error'); }
    }

    function renderPagination(total, limit) {
        const totalPages = Math.max(1, Math.ceil(total/limit));
        const start = (currentPage-1)*limit+1, end = Math.min(currentPage*limit, total);
        document.getElementById('paginationInfo').textContent = total>0 ? `${start}–${end} de ${total} registros` : '0 registros';
        const btn = (p,label,disabled,active) => `<button class="page-btn${active?' active':''}" onclick="goPage(${p})" ${disabled?'disabled':''}>${label}</button>`;
        let html = btn(1,'«',currentPage===1,false)+btn(currentPage-1,'‹',currentPage===1,false);
        for(let i=Math.max(1,currentPage-2);i<=Math.min(totalPages,currentPage+2);i++) html+=btn(i,i,false,i===currentPage);
        html += btn(currentPage+1,'›',currentPage===totalPages,false)+btn(totalPages,'»',currentPage===totalPages,false);
        document.getElementById('paginationControls').innerHTML = html;
    }

    function updateSortHeaders() {
        document.querySelectorAll('th[data-sort]').forEach(th => { const s=th.querySelector('.sort-icon'); if(s) s.textContent=th.dataset.sort===currentSort?(currentDir==='ASC'?'↑':'↓'):'↕'; });
    }

    function openModal() { document.getElementById('recId').value=''; document.getElementById('recDescricao').value=''; document.getElementById('recSituacao').value='a'; document.getElementById('modalTitle').textContent='Nova Forma de Pagamento'; document.getElementById('modal').classList.add('open'); setTimeout(()=>document.getElementById('recDescricao').focus(),100); }
    function closeModal() { document.getElementById('modal').classList.remove('open'); }

    async function editRecord(id) {
        try {
            const res=await fetch(`forma_pagamento.php?ajax=get&id=${id}`); const data=await res.json();
            if(!data.success) throw new Error(data.error);
    ...
    async function saveRecord() {
        ...
        const body=new FormData(); body.append('ajax','save'); body.append('id',document.getElementById('recId').value); body.append('descricao',descricao); body.append('situacao',document.getElementById('recSituacao').value);
        try { const res=await fetch('forma_pagamento.php',{method:'POST',body}); const data=await res.json(); if(!data.success) throw new Error(data.error); showToast(data.message,'success'); closeModal(); loadData(); } catch(e){showToast(e.message,'error');}
    }

    function openDelete(id,nome){deleteId=id;document.getElementById('deleteInfo').textContent=nome;document.getElementById('deleteModal').classList.add('open');}
    function closeDelete(){document.getElementById('deleteModal').classList.remove('open');deleteId=null;}
    async function confirmDelete(){if(!deleteId)return;const body=new FormData();body.append('ajax','delete');body.append('id',deleteId);try{const res=await fetch('forma_pagamento.php',{method:'POST',body});const data=await res.json();if(!data.success)throw new Error(data.error);showToast(data.message,'success');closeDelete();loadData();}catch(e){showToast(e.message,'error');}}
